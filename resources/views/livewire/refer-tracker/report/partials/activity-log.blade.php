                    ...
                    @foreach ($dayClicks as $click)
                        ...
                        >
                            ...
                            >
                                ...
                                <div
                                    class="mt-2 grid grid-cols-1 gap-x-4 gap-y-1 text-sm text-gray-800 sm:grid-cols-2"
                                >
                                    <p>
                                        <strong class="font-medium">
                                            UPI:
                                        </strong>
                                        {{ $this->maskUpi($click->upi) }}
                                    </p>
                                    @if ($campaign->is_extra_input_1_active)
                                        <p>
                                            <strong class="font-medium">
                                                {{ $campaign->extra_input_1_label }}:
                                            </strong>
                                            {{ $this->maskValue($click->extra_input_1) }}
                                        </p>
                                    @endif
                                    ...
                                    @if ($campaign->is_extra_input_2_active)
                                        <p>
                                            <strong class="font-medium">
                                                {{ $campaign->extra_input_2_label }}:
                                            </strong>
                                            {{ $this->maskValue($click->extra_input_2) }}
                                        </p>
                                    @endif
                                    ...
                                    @if ($campaign->is_extra_input_3_active)
                                        <p>
                                            <strong class="font-medium">
                                                {{ $campaign->extra_input_3_label }}:
                                            </strong>
                                            {{ $this->maskValue($click->extra_input_3) }}
                                        </p>
                                    @endif
                                </div>
                            </div>
                                ...
                                @foreach ($allCampaignEvents as $event)
                                    @php
                                        ...
                                        if ($conversion && $event->is_instant_pay_refer && $event->refer_amount > 0) {
                                            $earning = $conversion->earnings->where('type', App\Enums\EarningType::REFER)->first();
                                    ...
                                    >
                                        ...
                                        @if ($event->is_instant_pay_refer && $event->refer_amount > 0 && $isCompleted)
                                            <div
