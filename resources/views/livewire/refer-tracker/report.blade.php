...
>
    <div class="mx-auto max-w-6xl">
        ...
        @include('livewire.refer-tracker.report.partials.header')

        {{-- Filters: Search and date range controls --}}
        @include('livewire.refer-tracker.report.partials.filters')

        {{-- Main Content: Display stats and activity if results exist --}}
        @if ($hasResults)
            {{-- Detailed log of all clicks and their associated event statuses --}}
            @include('livewire.refer-tracker.report.partials.activity-log')
        @else
            {{-- Message shown when no data matches the current filters --}}
            @include('livewire.refer-tracker.report.partials.no-results')
        @endif

        {{-- Footer: Back link and contact info --}}
        @include('livewire.refer-tracker.report.partials.footer')
    </div>
</div>
